...
final class WorkspaceCommandBar {
    ...
    void render(WorkspaceLayout.Panel panel, WorkspaceState state, GpuParticleSystem particles, float fps,
        ...
        if (ImGui.begin("##workspace-command-bar", WINDOW_FLAGS)) {
            ...
            renderCenter(panel.width(), particles, paused, togglePause);
    ...
    private void renderLeft(float width, Runnable loadPreset, Runnable savePreset) {
        ...
        if (width >= 720.0f) {
            ...
            ImGui.sameLine();
            if (ImGui.button("Save##command-save", 64.0f, 28.0f)) {
                savePreset.run();
            }
        }
    }

    private void renderCenter(float width, GpuParticleSystem particles, boolean paused, Runnable togglePause) {
        float controlsWidth = width >= 720.0f ? 236.0f : 136.0f;
        ImGui.sameLine(Math.max(ImGui.getCursorPosX() + 8.0f, (width - controlsWidth) * 0.5f));
        ImGui.pushStyleColor(ImGuiCol.Button, UiPalette.ACCENT.vec4());
        if (ImGui.button(paused ? "Resume##command-pause" : "Pause##command-pause", 78.0f, 28.0f)) {
            togglePause.run();
        }
        ImGui.popStyleColor();
        if (width >= 720.0f) {
            ...
            if (ImGui.button("Step##command-step", 64.0f, 28.0f)) {
                particles.step();
            }
            ImGui.endDisabled();
            ImGui.sameLine();
            if (ImGui.button("Reset##command-reset-particles", 72.0f, 28.0f)) {
                particles.reset();
            }
        }
    }
    ...
    private void renderRight(float width, WorkspaceState state, GpuParticleSystem particles, float fps, boolean paused,
        ...
        if (width >= 1100.0f) {
            ImGui.text("%,d particles  |  %.0f FPS".formatted(particles.particleCount(), fps));
            ImGui.sameLine();
        }
        if (ImGui.button("...##command-overflow", 36.0f, 28.0f)) {
            ImGui.openPopup("command-overflow-menu");
        }
        if (ImGui.beginPopup("command-overflow-menu")) {
            if (width < 720.0f && ImGui.menuItem("Load...")) loadPreset.run();
            if (width < 720.0f && ImGui.menuItem("Save...")) savePreset.run();
            if (width < 720.0f) {
                ImGui.beginDisabled(!paused);
