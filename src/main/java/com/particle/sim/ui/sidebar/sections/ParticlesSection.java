...
final class ParticlesSection {
    ...
    void render(SimulationViewModel.Particles particles, SimulationViewActions.Particles actions) {
        ...
        if (metricsFitSideBySide(summaryWidth, tokens)) {
            float particleCardWidth =
                    Math.max(
                            tokens.primaryMetricMinimumWidth(),
                            (summaryWidth - tokens.spaceMd()) * 0.62f);
            Metric.card(
                    "particle-count",
                    "PARTICLES",
                    "%,d".formatted(particles.particleCount()),
                    particleCardWidth);
            ImGui.sameLine();
            Metric.card(
                    "group-count",
                    "GROUPS",
                    Integer.toString(particles.groupCount()),
                    Math.max(
                            tokens.secondaryMetricMinimumWidth(),
        ...
        Metric.row(
                "Capacity",
                "%,d / %,d".formatted(particles.particleCount(), particles.maximumParticleCount()));

        Text.divider();

        if (particles.particleCount() == 0) {
            Text.emptyState(
                    "No particles are active. Choose a spawn mode and add a population below.");
        }
        ...
        float pairWidth =
                Math.max(
                        tokens.pairedControlMinimumWidth(),
        ...
        if (IntegerInput.render(
                ...
                1_000,
                0,
                particles.maximumParticleCount(),
                inputWidth)) {
        ...
        if (Button.text(
                ...
                inlineCustomAction ? tokens.buttonWidthSm() : 0.0f,
                tokens.controlHeight(),
                remainingCapacity(particles) > 0 && customSpawnAmount.get() > 0)) {
    ...
    private void spawnButton(
            ...
            DesignTokens tokens) {
        boolean enabled =
                amount > 0 ? remainingCapacity(particles) > 0 : particles.particleCount() > 0;
        if (Button.text(
                label,
                "spawn-" + amount,
    ...
    static boolean customControlsFitInline(float availableWidth, DesignTokens tokens) {
        return availableWidth
                >= tokens.inputMinimumWidth() + tokens.buttonWidthSm() + tokens.spaceMd();
    }
    ...
    private static void renderCapacityStatus(
            SimulationViewModel.Particles particles, int requested) {
        ...
        if (remaining <= 0) {
            ...
            Text.helper(
                    "Only %,d spaces remain; the next add will be clamped.".formatted(remaining));
        } else {
