...
public final class Combo {
    ...
    public static boolean render(
            String label, String id, ImInt valueRef, String[] values, boolean enabled) {
        ImGui.pushFont(Fonts.medium());
        try {
            ImGui.textUnformatted(label);

            String preview =
                    isValidIndex(valueRef.get(), values) ? values[valueRef.get()] : "Select...";
            ...
            try {
                ...
                drawPreview(
                        drawList, origin, width, height, preview, hovered, focused, open, enabled);
                ...
                if (open) {
                    try {
                        for (int index = 0; index < values.length; index++) {
                            ...
                            if (ImGui.selectable(
                                    Button.itemLabel(
                                            values[index], "combo-option-" + id + "-" + index),
                                    selected)) {
                                valueRef.set(index);
                                changed = true;
                            }
    ...
    private static void drawPreview(
        ...
        drawList.addRect(
                origin.x, origin.y, maxX, maxY, borderColor, tokens.radiusMd(), 0, borderThickness);
        ...
        drawList.addLine(
                centerX - chevronWidth * 0.5f,
                centerY - direction * tokens.spaceXxs(),
                centerX,
                centerY + direction * tokens.spaceXxs(),
                chevronColor,
    ...
    static float previewTextWidth(float width, DesignTokens tokens) {
