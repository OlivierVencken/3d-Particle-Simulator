...
public class AttractionMatrix {
    ...
    public void setActiveValues(float[] values) {
        ...
        for (int row = 0; row < groupCount; row++) {
            for (int column = 0; column < groupCount; column++) {
                ...
                if (valueIndex < values.length) set(row, column, values[valueIndex]);
                valueIndex++;
            }
        }
    ...
    public void animatedMutation(boolean enabled) {
        if (animatedMutation == enabled) return;
        if (enabled) rememberState();
        animatedMutation = enabled;
    }

    public void advanceAnimation(float deltaTime) {
        if (!animatedMutation || !Float.isFinite(deltaTime) || deltaTime <= 0.0f) return;
    ...
    private void generatePredatorPrey(float variation) {
        generateFrom((row, column) -> {
            if (row == column) return 0.2f;
            if (column == (row + 1) % groupCount) return 0.8f;
            if (row == (column + 1) % groupCount) return -0.7f;
            return -0.08f;
        }, variation);
    }
    ...
    private void generateRockPaperScissors(float variation) {
        generateFrom((row, column) -> {
            ...
            if (rowFaction == columnFaction) return 0.35f;
    ...
    private void generateSparse(float variation) {
        forEachCell((row, column) -> {
            if (row == column) setPattern(row, column, 0.3f, variation);
            else if (random.nextFloat() < 0.2f) set(row, column, clamp(randomBase(-0.8f, 0.8f) + noise(variation)));
            else set(row, column, 0.0f);
        });
    }
    ...
    private void generateParasitism(float variation) {
        generateFrom((row, column) -> {
            if (row == column) return 0.15f;
            if (row % 2 == 0 && column == Math.min(row + 1, groupCount - 1)) return 0.8f;
            if (column % 2 == 0 && row == Math.min(column + 1, groupCount - 1)) return -0.65f;
            return -0.05f;
        }, variation);
    }
    ...
    private void generateRing(float variation, boolean closed) {
        generateFrom((row, column) -> {
            if (row == column) return 0.25f;
            boolean next = column == row + 1 || (closed && row == groupCount - 1 && column == 0);
            boolean previous = row == column + 1 || (closed && column == groupCount - 1 && row == 0);
            if (next) return 0.8f;
            if (previous) return 0.3f;
            return -0.12f;
        }, variation);
    }

    private void generateFrom(ValueGenerator generator, float variation) {
        forEachCell((row, column) -> setPattern(row, column, generator.value(row, column), variation));
    }
    ...
    private void mutateWithoutHistory(float amount) {
        ...
        forEachCell((row, column) -> set(row, column,
                attraction(row, column) + (random.nextFloat() * 2.0f - 1.0f) * amount));
    }
